...
get([ID,_,_,_,_,_],ID ).
get([_,Username,_,_,_,_],Username ).
get([_,_,Password,_,_,_],Password ).
get([_,_,_,FechaR,_,_],FechaR ).
get([_,_,_,_,ListaPubli,_],ListaPubli ).
get([_,_,_,_,_,ListaComent],ListaComent ).

%este "selector" obtiene la lista del usuario buscado dentro de la lista de usuarios
getUserLista([[Username,Password,Fecha,Puntaje,ListaPreguntas,ListaRespuestas]|RestoUsers],UserBuscado,Lista):-  
    (Username = UserBuscado, Lista=[Username,Password,Fecha,Puntaje,ListaPreguntas,ListaRespuestas]); getUserLista(RestoUsers,UserBuscado,Lista).
getUserLista([_|[Username,Password,Fecha,Puntaje,ListaPreguntas,ListaRespuestas]],UserBuscado,Lista):- (Username = UserBuscado, Lista=[Username,Password,Fecha,Puntaje,ListaPreguntas,ListaRespuestas]).
